...
- **Name**：「凡人修仙传」**行楷/楷体毛笔字族**（2026-09-01 产品主人提供书法体
  参考图；字体走浏览器本地字库，缺字回退 serif）+ 渐变裁字
  （`#A87B2F → #D9B45C → #B8433F`）+ 反色徽章 **BEAUTY**（2026-09-01 裁决：原
  「美人志」三字换英文）——亮=玄夜底 `#12121A` + 雾白字 `#FAF9F6`，暗=雾白底 +
  玄夜字（tgcf badge 反转结构）。
  备选：若浏览器缺行楷字体观感不佳，可改为手绘 SVG 字形或内嵌完整标题图。
