...
- 素材声明：12 张壁纸均为 **AI 生成同人图，非官方《凡人修仙传》素材，不标注生成工具**；
  龙标 mark/favicon 直接内嵌产品主人提供的龙标图（128px WebP），BEAUTY 徽章由
  HTML/CSS 绘制，萤火粒子由 CSS 伪元素与径向渐变绘制，「凡人修仙传」五字为
  原创手绘笔画字形 SVG——均不包含官方素材。源图 sha256 逐张记录于
...
- **Mark = 龙标**（2026-09-01 二次裁决：手绘 SVG 龙不够保真，改为直接内嵌产品
  主人提供的龙标图）：补边成方后 128px WebP q90 内嵌（5.8KB），data URL 同供
  mark 与 favicon。
- **Name**：「凡人修仙传」**手绘笔画字形 SVG**（2026-09-01 Plan B：行楷字体栈
  观感不足废弃；五组原创笔触字形共用鎏金→绯红渐变 ink `#A87B2F→#D9B45C→
  #B8433F`，不依赖字体环境）+ 反色徽章 **BEAUTY**（原「美人志」换英文）——
  亮=玄夜底 `#12121A` + 雾白字 `#FAF9F6`，暗=雾白底 + 玄夜字（tgcf badge 反转结构）。
