...
- 素材声明：12 张壁纸均为 **AI 生成同人图，非官方《凡人修仙传》素材，不标注生成工具**；
  掌天瓶 mark/favicon 直接内嵌产品主人提供的掌天瓶图（96px WebP），BEAUTY 徽章由
  HTML/CSS 绘制，萤火粒子由 CSS 伪元素与径向渐变绘制——均不包含官方素材。源图
  sha256 逐张记录于 `src/client/skins/meirenzhi/wallpapers.js` 头注释。
...
- **Mark = 掌天瓶**（2026-09-01 终版：龙标图试制后产品主人改回掌天瓶，直接内嵌
  其提供的掌天瓶图）：补边成方后 96px WebP q75 内嵌（1.6KB），data URL 同供
  mark 与 favicon。
- **Name**：「凡人修仙传」**渐变裁字原始方案**（`#A87B2F → #D9B45C → #B8433F`，
  粗体 700）+ 反色徽章 **BEAUTY**——亮=玄夜底 `#12121A` + 雾白字 `#FAF9F6`，
  暗=雾白底 + 玄夜字（tgcf badge 反转结构）。
  已否决两案：行楷/楷体字体栈（观感不足）、手绘笔画字形 SVG（小尺寸不可读）。
