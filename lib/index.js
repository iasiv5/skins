...
// src/host/personalization/zip.js
var ENTRY_LIMIT = 64;
var ARCHIVE_LIMIT = 80 * 1024 * 1024;
var ENTRY_SIZE_LIMIT = 20 * 1024 * 1024;
var MANIFEST_LIMIT = 256 * 1024;
var NAME_LIMIT = 120;
var ASSET_NAME = /^assets\/[a-z0-9_-]{1,80}\.(png|jpe?g|webp|gif)$/;
var EOCD = 101010256;
...
function crc32(buffer) {
  ...
  for (let index = 0; index < buffer.length; index += 1) {
    crc = crc >>> 8 ^ CRC_TABLE[(crc ^ buffer[index]) & 255];
  }
  return (crc ^ -1) >>> 0;
}
function dosDateTime(date = /* @__PURE__ */ new Date(0)) {
...
function writeStoreOnlyZip(entries) {
...
function readStoreOnlyZip(buffer) {
  ...
  for (let index = 0; index < countTotal; index += 1) {
    ...
    const method3 = buffer.readUInt16LE(cursor + 10);
    ...
    const name2 = centralNameBytes.toString("utf8");
    cursor += 46 + nameLength + extraLength + commentLength;
    if (versionNeeded !== 20) throw codedError("IMPORT_INVALID", `${name2}: unsupported version ${versionNeeded}`);
    if (flags !== 0) throw codedError("IMPORT_INVALID", `${name2}: unsupported flags 0x${flags.toString(16)} (encryption/descriptors rejected)`);
    if (method3 !== 0) throw codedError("IMPORT_INVALID", `${name2}: only store (method 0) entries are accepted`);
    if (compressedSize !== uncompressedSize) throw codedError("IMPORT_INVALID", `${name2}: store entry size mismatch`);
    if (compressedSize === 4294967295 || uncompressedSize === 4294967295 || localOffset === 4294967295) {
      throw codedError("IMPORT_INVALID", `${name2}: Zip64 rejected`);
    }
    if (extraLength !== 0 || commentLength !== 0) throw codedError("IMPORT_INVALID", `${name2}: extra fields and comments rejected`);
    if (nameLength > NAME_LIMIT) throw codedError("IMPORT_INVALID", `${name2}: name too long`);
    if (name2 === "manifest.json") {
    ...
    } else if (!ASSET_NAME.test(name2)) {
      throw codedError("IMPORT_INVALID", `${name2}: entry names must be manifest.json or assets/<safe-name>`);
    }
    if (assets.has(name2) || name2 === "manifest.json" && manifest !== null) {
      throw codedError("IMPORT_INVALID", `${name2}: duplicate entry`);
    }
    if (localOffset + 30 > eocdOffset || buffer.readUInt32LE(localOffset) !== LOCAL) {
      throw codedError("IMPORT_INVALID", `${name2}: local header missing`);
    }
    const localNameLength = buffer.readUInt16LE(localOffset + 26);
    const localExtraLength = buffer.readUInt16LE(localOffset + 28);
    if (localNameLength !== nameLength || !buffer.subarray(localOffset + 30, localOffset + 30 + localNameLength).equals(centralNameBytes)) {
      throw codedError("IMPORT_INVALID", `${name2}: local/central name mismatch`);
    }
    if (localExtraLength !== 0) throw codedError("IMPORT_INVALID", `${name2}: local extra fields rejected`);
    if (buffer.readUInt16LE(localOffset + 4) !== 20) throw codedError("IMPORT_INVALID", `${name2}: local/central version mismatch`);
    if (buffer.readUInt16LE(localOffset + 6) !== 0 || buffer.readUInt16LE(localOffset + 8) !== 0) {
      throw codedError("IMPORT_INVALID", `${name2}: local flags/method differ from store-only`);
    }
    if (buffer.readUInt32LE(localOffset + 14) !== crc || buffer.readUInt32LE(localOffset + 18) !== compressedSize || buffer.readUInt32LE(localOffset + 22) !== uncompressedSize) {
      throw codedError("IMPORT_INVALID", `${name2}: local/central CRC or size mismatch`);
    }
    const dataStart = localOffset + 30 + localNameLength + localExtraLength;
    const dataEnd = dataStart + uncompressedSize;
    if (dataEnd > centralOffset) throw codedError("IMPORT_INVALID", `${name2}: data intrudes on the central directory`);
    if (uncompressedSize > ENTRY_SIZE_LIMIT) throw codedError("IMPORT_TOO_LARGE", `${name2}: entry exceeds 20MB`);
    const data = buffer.subarray(dataStart, dataEnd);
    if (crc32(data) !== crc) throw codedError("IMPORT_INVALID", `${name2}: CRC mismatch`);
    for (const [start, end] of ranges) {
      if (dataStart < end && start < dataEnd) throw codedError("IMPORT_INVALID", `${name2}: overlapping entry data`);
    }
    ranges.push([dataStart, dataEnd]);
    localRecords.push([localOffset, dataEnd]);
    if (name2 === "manifest.json") manifest = Buffer.from(data);
    else assets.set(name2, Buffer.from(data));
  }
...
function createPersonalizationStore(options = {}) {
  ...
  const stagingDir = join2(dataDir, STAGING_DIR);
...
`, fs);
  ...
  function cleanupStaging() {
    ...
    for (const entry of fs.readdirSync(stagingDir)) {
      const dir = join2(stagingDir, entry);
      try {
        if (now() - fs.statSync(dir).mtimeMs < STAGING_TTL_MS) continue;
        for (const file of fs.readdirSync(dir)) fs.unlinkSync(join2(dir, file));
        fs.rmdirSync(dir);
      } catch {
      }
    }
  ...
  async function prepareImport(buffer) {
    ...
    if (manifest === null || typeof manifest !== "object" || Array.isArray(manifest) || manifest.formatVersion !== 1 || typeof manifest.skinId !== "string" || typeof manifest.fields !== "object" || manifest.fields === null || Array.isArray(manifest.fields) || !Array.isArray(manifest.assets)) {
    ...
    for (const asset of manifest.assets) {
      ...
      if (data === void 0) throw codedError("IMPORT_INVALID", `包内缺少 ${asset.file}`);
      if (sha256Hex(data) !== asset.sha256) throw codedError("IMPORT_INVALID", `${asset.file} 哈希不匹配`);
      listedFiles.add(asset.file);
    }
    for (const name2 of assets.keys()) {
      if (!listedFiles.has(name2)) throw codedError("IMPORT_INVALID", `${name2} 未在 manifest 中登记`);
    }
    const preparedMeta = /* @__PURE__ */ new Map();
    ...
    for (const [key, value] of Object.entries(manifest.fields)) {
      ...
      const verdict = validateOverride(manifest.skinId, key, value, void 0);
    ...
    const removeFields = [...knownKeys].filter((key) => current[key] !== void 0 && !(key in mappedFields));
    ...
    return enqueue(() => {
      ...
      try {
        ...
        for (const [key, value] of Object.entries(entry.mappedFields)) {
          const finalValue = value !== null && typeof value === "object" && typeof value.$asset === "string" ? mapping[value.$asset] : value;
          if (finalValue === void 0) continue;
...
var THEME_ARCHIVE_LIMIT = 80 * 1024 * 1024;
  ...
  try {
    ...
    const themeImportRoute = host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          if (action === "prepare") {
            const declared = Number(header2(request.headers, "content-length") ?? "0");
            ...
            return sendJson2(response, 200, await store.prepareImport(buffer));
          }
          if (action === "commit") {
            const body = await readJsonBody2(request);
            return sendJson2(response, 200, await store.commitImport(body));
          }
    ...
    const themeExportRoute = host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        if (match === null) return sendJson2(response, 400, { error: "skin id required", code: "UNKNOWN_SKIN" });
        try {
