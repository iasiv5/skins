...
function parseGif(buffer) {
  ...
  return {
    ...
    animated: null
    // frame counting is out of scope; GIF animation is allowed
  };
}
...
function parseWebp(buffer) {
  ...
  if (chunk === "VP8 ") {
    if (ascii(buffer, 20, 3) !== "*") return null;
    return {
      mime: "image/webp",
...
function readStoreOnlyZip(buffer) {
  ...
  for (let index = 0; index < countTotal; index += 1) {
    ...
    const name2 = buffer.subarray(cursor + 46, cursor + 46 + nameLength).toString("utf8");
    ...
    if (localNameLength !== nameLength) throw codedError("IMPORT_INVALID", `${name2}: local/central name mismatch`);
...
`, fs);
  ...
  async function uploadAsset(buffer, { displayName, declaredMime }) {
    ...
    return enqueue(() => {
      ...
      for (let attempt = 0; attempt < 5; attempt += 1) {
        id = ASSET_ID_PREFIX + randomUUID2().replaceAll("-", "");
        target = blobFileFor(id, extension);
        if (!fs.existsSync(target)) break;
      }
      const stagingPath = join2(stagingDir, randomUUID2());
      fs.mkdirSync(stagingPath, { recursive: true });
      const stagingFile = join2(stagingPath, "blob");
      try {
  ...
  async function confirmRecovery() {
    ...
    return enqueue(() => {
      ...
      for (const name2 of recovery.quarantine) {
        try {
