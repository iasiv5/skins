...
- `tgcf`（天官赐福 · 百无禁忌）是**非官方粉丝作品**，与《天官赐福》版权方无关联、未获授权；全部视觉为原创代码绘制 SVG（祥云灯笼阵 / 银蝶群 / 金线山水 / 红枫落雨），不含任何官方素材。暗色朱红鎏金、亮色素白金线，出厂标语「百无禁忌」。
...
每张皮肤卡右侧有齿轮按钮（有自定义时带圆点标记），在换肤弹层右侧展开该皮肤的个性化大面板（窄窗口时上下堆叠）：

- **可定制项**（`tgcf` 全量开放；`openbmc` / `uefi-harness` 开放壁纸）：壁纸（内置纹样 + 个人图库）、标语（中英）、面板不透明度、背景模糊、遮罩强度（单值，亮暗同度）。颜色、站点图标与标签页标题为皮肤固定设计，不再提供调节。
- **个人图库**：上传 PNG / JPEG / WebP / GIF（单张 ≤ 20MB、GIF ≤ 12MP；动画 WebP 与 SVG 不收），所有皮肤共用；删除被引用的图片或清空图库时会先列出全部受影响的皮肤与字段。
- **显式保存**：编辑期间界面实时预览，点「保存」才写入配置并跨标签页同步；「还原」一键放弃全部未保存修改（离线也可用）；带着未保存修改离开（点空白处 / 再点换肤按钮 / Esc / 收起面板 / 切换编辑目标——含在列表中点其他皮肤的卡片或齿轮）都会先确认。面板展开时点其他皮肤卡片，面板会随之切到该皮肤。
