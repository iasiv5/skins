...
- **可定制项**（三皮肤同一字段集：`tgcf` / `openbmc` / `uefi-harness`）：壁纸（内置画作 + 个人图库）、标语（中英）、面板通透度（0–100%，单值联动面板底色、遮罩与模糊三层：0% 纯壁纸完全透出，100% 完全遮蔽；各皮肤默认值锚定出厂视觉——tgcf 30%、openbmc/uefi 55%）。颜色、站点图标与标签页标题为皮肤固定设计，不再提供调节。
