...
- **Auto-save**: every change in the panel (wallpaper, slogan, opacity, blur, scrim) applies immediately and persists automatically after a brief debounce — no save button, no confirmation dialogs; Reset-to-default reverts everything in one click and saves automatically. Same click-to-apply experience as switching skins. While the panel is open, clicking another skin's card moves the panel to that skin.
