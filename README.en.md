...
- **Customizable fields** (fully open on `tgcf`; wallpaper-only on `openbmc` / `uefi-harness`): wallpaper (built-in artwork + a personal library), slogan (zh/en), panel translucency (0–100%; one value jointly drives the panel tint, the wallpaper scrim and the blur — 0% is pure, fully visible wallpaper, 100% fully hides it). Colours, the site icon and the tab title are fixed skin design and no longer adjustable.
